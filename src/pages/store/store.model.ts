...
export class StoreModel {
    map: google.maps.Map;
      ...
      map_places: Array<StorePlace> = [];
  
      search_query: string = '';
      search_places_predictions: Array<google.maps.places.AutocompletePrediction> = [];
  
      nearby_places: Array<StorePlace> = [];
  
      directions_origin: StorePlace = new StorePlace();
      ...
      addPlaceToMap(location: google.maps.LatLng, color: string = '#333333') : StorePlace {
          let _map_place = new StorePlace();
  
          _map_place.location = location;
          _map_place.marker = new google.maps.Marker({
        position: location,
        map: this.map,
        icon: StorePlace.createIcon(color)
  ...
  export class StorePlace {
      ...
      setIcon(color: string) : void {
          this.marker.setIcon('assets/icon/galler_icon.png');
